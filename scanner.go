...
func (ns *NetworkScanner) hostDiscovery() bool {
	...
	for _, port := range testPorts {
		...
		address := net.JoinHostPort(ns.results.Target, strconv.Itoa(port))
...
func (ns *NetworkScanner) generateRecommendations() {
	...
	vulnerablePorts := map[int]string{
		...
		139:  "NetBIOS - Potential information disclosure, consider disabling",
		445:  "SMB - Ensure latest patches applied, restrict access",
		1433: "SQL Server - Restrict access, use strong authentication",
		3389: "RDP - Use VPN or restrict access, enable NLA",
		5900: "VNC - Use strong passwords and encryption",
		8080: "HTTP Alt - Consider using HTTPS (443) instead",
	}

	// Check for insecure HTTP services
	for _, http := range ns.results.HTTPInfo {
		if http.Port == 80 || http.Port == 8080 {
			recommendations = append(recommendations, fmt.Sprintf("Port %d HTTP: Consider migrating to HTTPS for security", http.Port))
		}
		if http.Status == 401 {
			recommendations = append(recommendations, fmt.Sprintf("Port %d: Authentication required (good security practice)", http.Port))
		}
	}
	...
	if ns.results.Target == "192.168.0.1" || ns.results.Target == "192.168.1.1" || ns.results.Target == "10.0.0.1" {
		recommendations = append(recommendations, "Router detected: Ensure firmware is updated regularly")
		recommendations = append(recommendations, "Router detected: Change default admin credentials")
		recommendations = append(recommendations, "Router detected: Disable WPS if not needed")
	}

	// NetBIOS specific recommendations
	for _, port := range ns.results.OpenPorts {
		if port.Port == 139 || port.Port == 445 {
			recommendations = append(recommendations, "SMB/NetBIOS: Consider network segmentation")
			recommendations = append(recommendations, "SMB/NetBIOS: Monitor for unusual file access patterns")
			break
		}
	}
